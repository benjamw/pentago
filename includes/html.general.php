...
function get_header($meta = null) {
	...
	if ($show_menu) {
		if ( ! $menu_data) {
			...
			list($menu_data['in_vites'], $menu_data['out_vites'], $menu_data['open_vites']) = Match::get_invite_count($_SESSION['player_id']);
	...
	<aside id="nav">';
		...
		<nav id="mygames" class="box">
			...
			foreach ($game_data as $game) {
				...
				$html .= '
				<li class="'.$class.'"><a href="game.php?id='.$game['game_id'].$GLOBALS['_&_DEBUG_QUERY'].'">'.htmlentities($game['name'], ENT_QUOTES, 'ISO-8859-1', false).'</a></li>';
			}
