...
+---------------------------------------------------------------------------
|
|   > Game Player Extension module for Pentago
...
return false;
	...
	static public function get_maxed( )
	{
		...
		if (true || $invites_count_toward_max_games) {
			$query = "
				SELECT COUNT(MP.match_id) AS invite_count
					, PE.player_id
...
DROP TABLE IF EXISTS `pa_pa_player`;
